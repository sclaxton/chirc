...
void handleUser_PRIVMSG(msg *message, cli *client){
    ...
    if (bstrchr(recipNick, '#') == 0){
        ...
        if (channel){
            chnlUsrTbl channelUserTable = *channel->table;
            chnlUsr *user = findChannelUser(channelUserTable, client->nick, channel->lock);
            if (user){
                if (bstrchr(channel->modes, 'm') >= 0){
                    if (bstrchr(user->modes, 'o') >= 0 || bstrchr(client->modes, 'o') >= 0
                            || bstrchr(user->modes, 'v') >= 0){
                        sendPrivmsgChannel(channel, client, privMessage);
                    }
                    else{
        ...
        if (recipient){
            ...
            if (recipient->away){
                send_RPL_AWAY(client, recipient->nick, recipient->away);
            }
        }
        else {
            send_ERR_NOSUCHNICK(client, recipNick);
...
void handleUser_NOTICE(msg *message, cli *client){
    ...
    if (bstrchr(recipNick, '#') == 0){
        ...
        if (channel){
            ...
            if (findChannelUser(channelUserTable, client->nick, channel->lock)){
                if (bstrchr(channel->modes, 'm') >= 0){
                    if (bstrchr(user->modes, 'o') >= 0 || bstrchr(client->modes, 'o') >= 0
                            || bstrchr(user->modes, 'v') >= 0){
                        sendNoticeChannel(channel, client, privMessage);
                    }
                }
                else {
                    sendNoticeChannel(channel, client, privMessage);
        ...
        cli *recipient = findUser(table, recipNick, tableLock);
        if (recipient){
            sendMessage(getFullUserId(client), recipient, "NOTICE", recipNick, privMessage);
        }
    }
}
...
void changeTopic(chnl *channel, cli *sender, bstring newTopic){
    pthread_rwlock_t *lock = channel->lock;
    ...
    HASH_ITER(hh, *channel->table, curr, next){
        sendMessage(getFullUserId(sender), curr->user, "TOPIC", channel->name, newTopic);
    }
    pthread_rwlock_unlock(lock);
}
...
void handleUser_TOPIC(msg *message, cli *client){
    ...
    if (existingChannel){
        ...
        if (existingUser){
            ...
            if (newTopic){
                if (bstrchr(existingChannel->modes, 't') >= 0){
                    if (bstrchr(existingUser->modes, 'v') >= 0 || bstrchr(client->modes, 'o') >= 0){
                        changeTopic(existingChannel, client, newTopic);
                    }
                    else {
                        send_ERR_CHANOPRIVSNEEDED(client, channelName);
                    }
                }
                else {
                        changeTopic(existingChannel, client, newTopic);
...
void handleUser_AWAY(msg *message, cli *client){
    ...
    if (params){
        bstring away = params->entry[0];
        client->away = away;
        send_RPL_NOWAWAY(client);
    }
    else {
        client->away = NULL;
        send_RPL_UNAWAY(client);
    }
